...
from project_week5.lanenet.model import utils
...
class TuSimpleDataset(Dataset):
    def __init__(self, dataset_dir, phase, size=(512, 288), transform=None):
        ...
        :param phase: 数据集类型 [train, val, test] 其中train， val为同一个数据集划分开所得
        :param size: 统一的图片大小
        :param transform: 变换
        """
        ...
        if phase == 'train' or phase == 'val':
            ...
            self.image_list = []
            self.lanes_list = []
            # 根据json解析出数据集(图片路径及ground truth(有的话))
        ...
        elif phase == 'test_extend':  # to load extra frames which have no lane annotation
            task_file = os.path.join(dataset_dir, 'test_tasks_0627.json')
            try:
                self.image_list = []
