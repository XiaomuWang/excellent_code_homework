...
class FCNs(nn.Module):
    ...
    def __init__(self, pretrained_net, num_classes=2):
        super().__init__()
        self.pretrained_net = pretrained_net
        self.relu = nn.ReLU(inplace=True)
    ...
    def forward(self, x):
        out2, out3, out4, out5 = self.pretrained_net(x)
        # x5 = output['x5']  # size=(N, 512, x.H/32, x.W/32)
        x4 = out5  # size=(N, 512, x.H/16, x.W/16)
        x3 = out4  # size=(N, 256, x.H/8,  x.W/8)
        x2 = out3  # size=(N, 128, x.H/4,  x.W/4)
        x1 = out2  # size=(N, 64, x.H/2,  x.W/2)
...
if __name__ == "__main__":
    ...
    fcn_model = FCNs(pretrained_net=resnet50_model, num_classes=n_class)
