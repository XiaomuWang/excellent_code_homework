...
'''
# 因为预训练模型下载太慢，所以就没有加载预训练模型，最后 看看结果再说吧
# 结果发现验证集的各项指标上串下跳的，也不知道为啥
# 这一套实验下来，我发现最后最好的是resnet50+fcn+lr+data这个组合。其中resnet50表示backbone，fcn表示方法，
# lr表示对学习率进行动态修改，data表示只将（255，0，255）的真值作为正类，其他的都变成负类。
import torch
...
IMG_W = 800
SAVE_INTERVAL = 5
MODEL_CKPT_DIR = "./ckpt/segnet/"
FIGURE_DIR = "./figures/segnet/"
WARMUP_STEPS = 88
