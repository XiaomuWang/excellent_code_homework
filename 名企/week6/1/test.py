...
from utils import cluster_embed, fit_lanes, sample_from_curve, sample_from_IPMcurve, generate_json_entry, \
    get_color
...
if __name__ == '__main__':
    ...
    batch_size = 1
    num_workers = 4
    #whether to perform Inverse Projective Mapping(IPM) before curve fitting
    ipm=False
    #是否显示图像
    show_img=True
    #是否保存图像
    ...
    test_set = tusimple_dataset(data_dir, phase='test')
    ...
    with torch.no_grad():
        ...
        checkpoint = torch.load('./check_point/ckpt_2020-10-15_22-16-25_20/ckpt_2020-10-15_22-16-25_epoch-20.pth')
        ...
        for step in range(num_test):
            ...
            preds_bin_expand_batch = pred_bin_batch.view(
                pred_bin_batch.shape[0] * pred_bin_batch.shape[1] * pred_bin_batch.shape[2] * pred_bin_batch.shape[3])
            ...
            for idx in range(batch_size):
                ...
                pred_bin_rgb = pred_bin_batch[idx].repeat(3, 1, 1).cpu().numpy().transpose(1, 2, 0).astype(
                    np.uint8) * 255
                ...
                if show_img:
                    if ipm:
                        ...
                        for idx, inst in enumerate(curves_pts_IPM):
                            ...
                            print(curve_sample_IPM.shape,rgb_IPM.shape,pred_inst_IPM_rgb.shape)
                            print(curve_sample_IPM.flags, rgb_IPM.flags, pred_inst_IPM_rgb.flags)
                        ...
                        for idx, inst in enumerate(curves_pts_pred):
                            if inst.ndim == 2:
                                ...
                                cv2.polylines(rgb.copy(), [inst.astype(np.int32)], False, (0, 0, 255), 2)
                                cv2.polylines(pred_bin_rgb.copy(), [inst.astype(np.int32)], False, (0, 0, 255), 2)
                                cv2.polylines(pred_inst_rgb.copy(), [inst.astype(np.int32)], False, (0, 0, 255), 2)
                ...
                print('{}    Epoch:{}  Step:{}  Time:{:5.1f}  '
                      ...
                      .format(train_start_time,  epoch, step, time_run * 1000,
        ...
        with open(f'output/test_pred-{train_start_time}.json', 'w') as f:
